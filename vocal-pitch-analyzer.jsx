...
import {
  ...
  frequencyToNote, detectPitch, noteToColor,
...
function PitchHistoryCanvas({ history }) {
  ...
  useEffect(() => {
    ...
    if (history.length < 2) {
      ...
      ctx.fillText("노래를 시작하면 피치 히스토리가 표시됩니다", w / 2, h / 2);
      return;
    }

    // Y축 범위: MIDI 36(C2) ~ 84(C6) 를 기본으로 하되, 데이터에 따라 조정
    ...
    ctx.textAlign = "center";
    const totalSec = history.length * 0.05; // ~20fps 기준 ~50ms 간격
          ...
          {isActive && currentNote ? (
            ...
            <div className="space-y-3 py-4">
              ...
              <p className="text-gray-600 text-sm">
                {isActive ? "소리를 내어 보세요..." : "Start Monitoring을 눌러 시작하세요"}
              </p>
            </div>
          )}
        </section>
